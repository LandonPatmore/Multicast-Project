	...
	public void run() {
		...
		while (true) {
			...
			if (receivedPacket != null) {
				// decrypt payload of incoming packet
				byte[] decrypted = null;
				try {
					decrypted = AES.decryptByteArray(receivedPacket.getData(), AES.TEST_PASSWORD);
				} catch (InvalidKeyException e) {
					// wrong password
					// TODO: Notify that password is wrong
					System.out.println("Wrong password");
				} catch (Exception e) {
					// ignored
				}
				receivedPacket.setData(decrypted);
